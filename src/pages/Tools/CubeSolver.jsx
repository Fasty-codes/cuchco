...
import { FiHome, FiRefreshCw, FiRotateCcw, FiPlay, FiSkipBack, FiSkipForward, FiChevronLeft, FiChevronRight, FiZap } from "react-icons/fi";
  ...
  return (
    <div className="cube-page">
      ...
      <div className="cube-topbar">
        <a href="/" className="cs-tb-home" title="Home"><FiHome size={17}/></a>
        <div className="cs-tb-divider"/>
        <span className="cs-tb-brand">CUCHCO</span>
        <span className="cs-tb-slash">/</span>
        <span className="cs-tb-title">CUBE SOLVER</span>
        <div style={{flex:1}}/>
        <button className={`cs-mode-btn${mode==="edit"?" cs-mode-on":""}`} onClick={()=>setMode("edit")}>Edit Colors</button>
        {solution && <button className={`cs-mode-btn${mode==="solve"?" cs-mode-on":""}`} onClick={()=>setMode("solve")}><FiPlay size={12}/> Solution</button>}
        <a href="/cube" className="cs-tb-back">← Cube</a>
      </div>
        ...
        <div className="cube-left">
          ...
          <div className="panel-section">
            ...
            <div className="action-btns">
              <button className="action-btn scramble" onClick={handleScramble}><FiRefreshCw size={13}/> Scramble</button>
              <button className="action-btn reset"    onClick={handleReset}><FiRotateCcw size={13}/> Reset</button>
              <button
                className={`action-btn solve${solving?" loading":""}`}
                onClick={handleSolve}
                disabled={solving}
              >
                {solving ? <><FiZap size={13}/> Solving…</> : <><FiZap size={13}/> Solve It!</>}
              </button>
            </div>
              ...
              {solution.moves.length > 0 && playback && (
                <div className="playback-bar">
                  ...
                  <div className="playback-controls">
                    <button className="pb-btn" onClick={goToStart} title="Start"><FiSkipBack size={14}/></button>
                    <button className="pb-btn" onClick={stepBack}  title="Back" disabled={playback.step<0}><FiChevronLeft size={14}/></button>
                    <button className="pb-btn pb-play" onClick={stepForward} title="Next" disabled={playback.step>=playback.moves.length-1}><FiChevronRight size={14}/></button>
                    <button className="pb-btn" onClick={goToEnd}   title="End"><FiSkipForward size={14}/></button>
                  </div>
        ...
        <div className="cube-right">
          ...
          {!solution && (
            <div className="right-placeholder">
              <div className="rp-icon"><FiZap size={32}/></div>
              <p>Paint your cube, then hit <strong>Solve It!</strong></p>
            </div>
          )}
        </div>
      </div>
